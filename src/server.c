    ...
    while (1) {
        ...
        if (fp == NULL || ServerPathDepth(path) < 1) {
            ...
            if(StringEq(extension,StringFrom("c",&sarena))) {
                ...
                if(!fp) {
                    char* command = StringToChar(StringFormat(&sarena,StringFrom("cd /myserver && gcc -O3 %S -o %s -lm -I/usr/include/postgresql -lpq",&sarena),strippedPath,binpath),&sarena);
                    printf("command: %s\n",command);
                ...
                if(fp) {
                    fclose(fp);
                    char* command = StringToChar(StringFormat(&sarena,StringFrom("cd /myserver && chmod +x %s && %s %S '%S' '%S' ",&sarena),binpath,binpath,method,path,content),&sarena);
                    printf("command: %s\n",command);
